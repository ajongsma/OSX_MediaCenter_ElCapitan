...
-- https://github.com/kai-h/Kerio-Connect-Webapp-Configurator
...
set theReverseDNSName to "nl.cutthecord."

-- If you're changing names etc, you shouldn't have to modify anything below this line...

-- Here we will prompt for the information specific to the current Kerio Connect installation.
-- This information could probably be extracted from mailserver.conf automatically
-- however AppleScript isn't the easiest language to process text with, so I'll take
-- the easy way out and just ask for the relevant information
...
display dialog "Please enter the mail server's fully qualified hostname (e.g. mail.example.com)" buttons {"OK"} default button "OK" default answer "plexrequest.cutthecord.nl"
set theHostname to text returned of the result
display dialog "Please enter the HTTP port for Kerio Connect (e.g. 8003)" buttons {"OK"} default button "OK" default answer "8003"
...
display dialog "Please enter the full path to the Kerio Connect mailserver.conf file" buttons {"OK"} default button "OK" default answer "/private/etc/apache2/httpd.conf"
...
set theApacheFolder to "/Users/Andries/Documents/Github/OSX_MediaCenter/scripts/"
...
<dict> 
    ...
    <key>displayName</key>
    <string>" & theHTTPName & " webapp</string>
...
<dict> 
    ...
    <key>displayName</key>
    <string>" & theHTTPName & " SSL webapp</string>
    <key>installationIndicatorFilePath</key>
    <string>" & theMailserverConf & "</string>
    <key>sslPolicy</key>
    <integer>0</integer>
